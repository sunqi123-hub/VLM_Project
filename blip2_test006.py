# 改58行
# sample500-In_ADM_Results       ./SaveFineTune/LDM-train-all-aligned-epochs00020     ./SaveFineTune/LDM-train-epochs3        ./weights/ldmFineTune      ./test_debug.csv
# 测试多个数据集示例    ADM  DDPM  Diff-ProjectedGAN   Diff-StyleGAN2  IDDPM   LDM  PNDM  ProGAN  ProjectedGAN  StyleGAN
"""
python blip2_test006.py \
    --model_path ./SaveFineTune/LDM-train-aligned--epochs000020/epoch020 \
...
python blip2_test006.py \
    --model_path ./SaveFineTune/LDM-train-aligned-epochs005/epoch005 \
    --dataset ./data/Test_CSV/test_LDM.csv \
    --num_samples 500
"""
...
# 定义目标保存目录（核心修改点1）
# ===========================
TARGET_SAVE_DIR = "./Test-Results/In_LDM_Results/sample500-0020-aligned-In_LDM_Results"     # In_ADM_Results/In_ADM_Results/sample500-In_ADM_Results      all-In_LDM_Results
# 确保目标目录存在，不存在则创建
        ...
        try:
            ...
            with torch.no_grad():
                generated_ids = model.generate(
                    **inputs,
                    # max_new_tokens=1,
                    max_new_tokens=3,  # 从 1 改为 3，允许模型完整生成 "fake"/"real"   后改
                    # num_beams=3,
                    num_beams=5,  # beam 数略增   后改
                    do_sample=False,
                    # temperature = 0.7  # 降低采样温度  后加
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base_model", type=str,
                        default="./blip2-opt-2.7b",   #/root/autodl-tmp/project/VLM-DETECT-main
