...
from gnn_cot import (
    StructuredCoTReward,
    build_gnn_cot_head,
    extract_blip2_vision_tokens,
    infer_blip2_vision_dim,
    save_gnn_cot_head,
)
...
def train(opt):
    ...
    use_structured_cot = opt.use_cot or (opt.use_gnn_cot and not opt.gnn_short_answer)
    ...
    if opt.use_gnn_cot:
        ...
        if opt.gnn_visual_feature_dim <= 0:
            opt.gnn_visual_feature_dim = infer_blip2_vision_dim(model)
        print(
            f"[INFO] GNN visual tokens: enabled={opt.gnn_use_vision_tokens}, "
            f"feature_dim={opt.gnn_visual_feature_dim}, forensic_nodes={opt.gnn_forensic_nodes}, "
            f"semantic_top_k={opt.gnn_semantic_top_k}"
        )
        gnn_head = build_gnn_cot_head(
            ...
            visual_feature_dim=opt.gnn_visual_feature_dim,
            forensic_nodes=opt.gnn_forensic_nodes,
            semantic_top_k=opt.gnn_semantic_top_k,
        ).to(device)
        if use_structured_cot:
    ...
    collate = lambda batch: collate_fn(batch, processor, use_cot=use_structured_cot)
    dataloader = DataLoader(
        dataset,
        batch_size=opt.batch_size,
        shuffle=True,
    ...
    print(f"[INFO] 训练 prompt: \"{COT_PROMPT_TEXT if use_structured_cot else PROMPT_TEXT}\"")
    ...
    for epoch in range(opt.epochs):
        ...
        for step, batch in progress_bar:
            ...
            if gnn_head is not None:
                vision_embeds = (
                    extract_blip2_vision_tokens(model, pixel_values)
                    if opt.gnn_use_vision_tokens
                    else None
                )
...
def parse_args():
    ...
    parser.add_argument("--gnn_short_answer", action="store_true",
                        help="Use the GNN branch with the short fake/real target first; useful for isolating accuracy gains from innovation point one.")
    ...
    parser.add_argument("--gnn_visual_feature_dim", type=int, default=0,
                        help="BLIP2 vision hidden size. Use 0 to infer from the loaded base model.")
    parser.add_argument("--gnn_forensic_nodes", type=int, default=8,
                        help="Learnable forensic concept nodes linked to spatial/frequency nodes.")
    parser.add_argument("--gnn_semantic_top_k", type=int, default=4,
                        help="Non-local kNN edges per visual/frequency node based on graph feature similarity.")
    parser.add_argument("--gnn_use_vision_tokens", action="store_true", default=True,
                        help="Use frozen BLIP2 vision tokens as graph spatial node features.")
    parser.add_argument("--gnn_no_vision_tokens", action="store_false", dest="gnn_use_vision_tokens",
                        help="Disable BLIP2 vision-token features and fall back to pixel statistics only.")
